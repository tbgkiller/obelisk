...
# There is deliberately no generate_ini() here, and adding one back would be a mistake.
#
# There used to be. It rendered GameUserSettings.ini and Game.ini from the schema and
# headed them "Generated by Obelisk - do not edit", which is the exact opposite of what
# these files are: somebody else's, with a hand-written banner, a mod's own section, and
# a key repeated seventeen times. Obelisk models a fraction of what is in them. Anything
# it does not model, a from-scratch renderer deletes.
#
# It was never wired to anything, which is the only reason this cluster's 167 unmodelled
# keys survived being migrated. It stayed in the file looking like the obvious function
# to call from a save path.
#
# Writing to those files goes through ini.merge_file() and gamecfg.apply(), which edit
# the lines they were asked to and leave everything else alone. See docs/GAME-SETTINGS.md.
