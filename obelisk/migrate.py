...
def master_instance(order, last=LAST):
    """Which map downloads first *while a migration is running*.

    In a settled cluster that is whichever map the operator put at the top of the list -
    nothing enforces which map that is. The map a migration moves last deliberately
    moves last, so for part of a rolling migration it has not moved yet - and a map that
    finds a new build published waits for a download that has nothing to start it,
    forever, looking exactly like a slow start. That cost a map twenty minutes of
    silence before anyone learned it was never going to come up.
...
def build_gate(installed, available, master_running):
    ...
    safe turns entirely on whether the map that downloads first is up: with it, the
    cluster updates the way it is designed to; without it, every other map stalls on a
    download that never starts. Refusing here costs seconds. Finding out from a health
    check costs twenty minutes per map, and says "never became healthy" rather than
    why.
    """
    if not installed or not available or str(installed) == str(available):
        return True, "the installed build is current"
    if master_running:
        return True, ("build %s is out and %s is installed; the map that downloads "
                      "first is running and will fetch it" % (available, installed))
    return False, (
        "A new server build is out (%s installed, %s available) and the map that "
        "downloads first is not running, so this map would wait for it and never start. "
        "Either pre-stage the new build before cutting over, or pin this run to the "
        "installed build and update the whole cluster together once that map has "
        "migrated."
        % (installed, available))
