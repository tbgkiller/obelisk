...
def render_maps_editor(store, running=False, refusal="", refusal_at="maps",
    ...
    return ('<fieldset id=maps><legend>Maps</legend>%s%s%s'
            ...
            'rather than all fetching the same thing at once. It keeps that job for '
            'later updates too, so the rest stall if it is not running when a new '
            'build lands. Ports are assigned down this list, so adding to the end '
            'leaves everyone else where they are and reordering does not.</div>'
