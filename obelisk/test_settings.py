...
check("first map downloads first, so the others depend on it", "depends_on" in yml and yml.count("depends_on") == 2)
...
# The restart policy is a setting now, and it defaults to `no`. A DoExit does not
# leave a map down while a policy is there - POK reads the missing server process as a
# self-restart and exits the container so that the policy will bring it back, which is
# the loop observed at The Center on 2026-09-18.
check("the ARK services get the default restart policy, which is no",
      yml.count("restart: no") == 3 and "unless-stopped" not in yml, yml)
check("every map gets an instance name", yml.count("INSTANCE_NAME:") == 3, yml)
check("first map gets POK's MASTER role", 'UPDATE_COORDINATION_ROLE: "MASTER"' in yml)
