...
rejects("rejects a stacking mod that isn't first", "mod_ids", "940003,929110,929420")
check("accepts a stacking mod first", validate("mod_ids", "929110,940003") == "929110,940003")
check("mod list tolerates spacing", validate("mod_ids", " 929110 , 940003 ") == "929110,940003")
...
rejects("rejects a bad clock time", "wipe_times", "4:30")
rejects("rejects hour 25", "wipe_times", "25:00")
check("accepts wipe times", validate("wipe_times", "04:30, 16:30") == "04:30,16:30")
...
rejects("rejects an empty admin password", "admin_password", "")
...
# ---- generation
st = new_store()
st.patch({"admin_password": "pw", "mod_ids": "929110,940003", "motd": "hello"})
st.patch({"mem_limit": "32g"}, map_name="astraeos")
env = generate_env(st)
check("env has the mod list", "MOD_IDS=929110,940003" in env)
...
st.patch({"wipe_times": "04:30,16:30"})
env = generate_env(st)
check("wipe schedule still reaches the running bot", "WIPE_TIMES=04:30,16:30" in env, env)
