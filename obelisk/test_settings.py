...
# No restart policy, on purpose. A DoExit does not leave a map down - POK reads the
# missing server process as a self-restart and exits the container so that a policy
# will bring it back, which is the loop observed at The Center on 2026-09-18. With no
# `restart:` key Docker's default of `no` applies and the deliberate exit is the end.
check("the ARK services carry no restart policy at all", "restart:" not in yml, yml)
